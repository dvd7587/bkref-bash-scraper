...
do
    ...
    PAGEPATH="leagues/NBA_"$YEAR"_games-"$MONTH".html" ; 
    wget "$BKREFURL""$PAGEPATH" -O - 2>> $LOGFILE | grep 'data-stat="box_score_text"' >> $TXTFILE
    grep boxscores $TXTFILE | sed "s:<tbody>::" | cut -d "<" -f "22-30" | cut -d '"' -f 2 > $CSVFILE
    sed -i -e "s_/boxscores_"$BKREFURL"boxscores_" $CSVFILE
done
